...
/**
 * Template Name:  website development AU page
 */
?>
<!DOCTYPE html>
<html lang="en-US">
...
<head>
 ...
 <style>:root{--blue:#1246BE;--blue-dark:#0a1f5c;--blue-mid:#1a56db;--blue-light:#3b82f6;--cyan:#06b6d4;--pale:#EEF4FF;--border:#c7d7ff;--white:#fff;--g50:#f8faff;--g100:#f0f4ff;--g200:#e2e8f0;--g600:#475569;--g700:#334155;--g900:#0f172a;--green:#10b981;--orange:#f59e0b;--purple:#7c3aed;--shsm:0 2px 8px rgba(18,70,190,.08);--shmd:0 8px 32px rgba(18,70,190,.12);--shlg:0 20px 60px rgba(18,70,190,.18);--r:12px;--rl:20px;}
section#why-us .section-title:after{left:0;right:auto;}
...
.hero-form-card i{position:absolute;left:14px;top:38px;color:var(--blue-light);font-size:15px;}
.form-guarantee i{color:var(--green);}
.form-guarantee{position:relative;}
.form-guarantee i{position:static;}

/* ── HEADER ── */
#contact-form{scroll-margin-top:150px;}
...
/* ── HERO ── */
.hero{background:linear-gradient(135deg,var(--blue-dark) 0%,var(--blue-primary) 60%,var(--blue-accent) 100%);position:relative;overflow:hidden;padding:70px 0 80px;margin-top:100px;}
...
.hero-trust-item{display:flex;align-items:center;gap:4px;color:rgba(255,255,255,.8);font-size:13px;}
...
h2.section-title{font-size:38px;font-weight:600;color:var(--blue-dark);line-height:1.25;margin-bottom:16px;padding-bottom:0}
...
.section-title.text-md-left::after{left:0;right:auto;}
...
.svc-icon{width:56px;height:56px;border-radius:14px;background:var(--pale);display:flex;align-items:center;justify-content:center;font-size:24px;margin-bottom:18px;color:var(--blue);transition:.3s;}
...
/* ═════════════════════════════════════════════ */
.shopify-section{background:linear-gradient(135deg,#f0fff4 0%,#e6fff0 50%,#f0fff4 100%);position:relative;overflow:hidden}
.shopify-section::before{content:'';position:absolute;top:-100px;right:-100px;width:400px;height:400px;background:radial-gradient(circle,rgba(149,191,71,.15),transparent 70%);border-radius:50%}
.shopify-section::after{content:'';position:absolute;bottom:-100px;left:-100px;width:300px;height:300px;background:radial-gradient(circle,rgba(6,182,212,.1),transparent 70%);border-radius:50%}
.shopify-grid{display:grid;grid-template-columns:1fr 1fr;gap:60px;align-items:center;position:relative;z-index:1}
.shopify-badge{display:inline-flex;align-items:center;gap:8px;background:#95BF47;color:#fff;padding:8px 18px;border-radius:30px;font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:1px;margin-bottom:14px}
.shopify-title{font-size:38px;font-weight:800;color:var(--blue-dark);line-height:1.25;margin-bottom:16px}
.shopify-title span{color:#6B9B1E}
.shopify-desc{font-size:15px;color:var(--g600);line-height:1.75;margin-bottom:24px}
.shopify-features{display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:32px}
.sf-item{display:flex;gap:12px;align-items:flex-start}
.sf-check{width:24px;height:24px;min-width:24px;background:#95BF47;border-radius:6px;display:flex;align-items:center;justify-content:center;color:#fff;font-size:11px;margin-top:2px}
.sf-item h4{font-size:13px;font-weight:700;color:var(--g900);margin-bottom:2px}
.sf-item p{font-size:11px;color:var(--g600);line-height:1.5}
.shopify-actions{display:flex;gap:14px;flex-wrap:wrap;margin-bottom:20px}
.shopify-trust{display:flex;gap:24px;flex-wrap:wrap}
.shopify-trust span{display:flex;align-items:center;gap:6px;font-size:12px;color:var(--g600);font-weight:500}
.shopify-trust i{color:#95BF47}
.shopify-visual{position:relative}
.shopify-main-card{background:var(--white);border-radius:var(--rl);padding:40px;box-shadow:var(--shmd);border:2px solid rgba(149,191,71,.2);text-align:center;position:relative;overflow:hidden}
.shopify-main-card::before{content:'';position:absolute;top:0;left:0;right:0;height:5px;background:linear-gradient(90deg,#95BF47,#6B9B1E,#95BF47)}
.shopify-logo-big{font-size:72px;margin-bottom:20px;display:block}
.shopify-card-title{font-size:20px;font-weight:800;color:var(--blue-dark);margin-bottom:8px}
.shopify-card-sub{font-size:13px;color:var(--g600);margin-bottom:24px;line-height:1.6}
.shopify-stats{display:grid;grid-template-columns:repeat(3,1fr);gap:16px;margin-bottom:24px}
.sstat{background:var(--pale);border-radius:12px;padding:16px 8px}
.sstat strong{display:block;font-size:24px;font-weight:800;color:var(--blue)}
.sstat span{font-size:11px;color:var(--g600)}
.shopify-platforms{display:flex;justify-content:center;gap:20px;flex-wrap:wrap}
.sp-badge{background:rgba(149,191,71,.1);padding:6px 14px;border-radius:20px;font-size:12px;font-weight:600;color:#6B9B1E;display:flex;align-items:center;gap:6px}
.sp-badge i{font-size:14px}
.btn-shopify{background:linear-gradient(135deg,#95BF47,#6B9B1E);color:#fff;padding:14px 32px;border-radius:8px;font-size:15px;font-weight:700;border:none;cursor:pointer;text-decoration:none;display:inline-flex;align-items:center;gap:8px;transition:.3s;box-shadow:0 4px 20px rgba(149,191,71,.4);}
.btn-shopify:hover{background:#fff;color:#6B9B1E;text-decoration:none;}
.shopify-float{position:absolute;background:var(--white);border-radius:12px;padding:14px 20px;box-shadow:var(--shmd);display:flex;align-items:center;gap:12px;min-width:180px;z-index:2}
.shopify-float.tr{top:-10px;right:-20px;border-left:4px solid #95BF47}
.shopify-float.bl{bottom:-10px;left:-20px;border-left:4px solid var(--blue)}
.sf-badge-icon{font-size:28px}
.sf-badge-text strong{display:block;font-size:16px;font-weight:800;color:var(--blue)}
.sf-badge-text span{font-size:11px;color:var(--g600)}

/* AUSTRALIA MAP SECTION */
.au-bg{background:var(--pale)}
.au-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:20px}
.au-card{background:var(--white);border-radius:var(--r);padding:24px 20px;text-align:center;border:1.5px solid var(--border);transition:.3s;cursor:default}
.au-card:hover{background:var(--blue);border-color:var(--blue);transform:translateY(-4px)}
.au-card:hover *{color:#fff!important}
.au-card .au-icon{font-size:32px;margin-bottom:10px;display:block}
.au-card h4{font-size:15px;font-weight:700;color:var(--blue-dark);margin-bottom:6px}
.au-card p{font-size:12px;color:var(--g600);line-height:1.5}
.au-card img{max-width:32px;}

/*end */

/* TECH STACK */
.tech-bg{background:var(--g100)}
.tech-grid{display:grid;grid-template-columns:repeat(6,1fr);gap:16px}
.tech-card{background:var(--white);border-radius:var(--r);padding:24px 16px;text-align:center;border:1.5px solid var(--border);transition:.3s;cursor:default}
.tech-card:hover{transform:translateY(-4px);box-shadow:var(--shmd);border-color:var(--blue)}
.tech-card .ticon{font-size:36px;margin-bottom:10px;display:block}
.tech-card h4{font-size:13px;font-weight:700;color:var(--blue-dark);margin-bottom:4px}
.tech-card p{font-size:11px;color:var(--g600)}

/* PLATFORM CARDS */
.platform-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:28px}
.platform-card:hover i{color:#fff;}
.platform-card{background:var(--white);border-radius:var(--rl);padding:36px 28px;border:1.5px solid var(--border);transition:.3s;position:relative;overflow:hidden;text-align:center}
.platform-card::before{content:'';position:absolute;top:0;left:0;right:0;height:4px;background:linear-gradient(135deg,var(--blue),var(--cyan));transform:scaleX(0);transform-origin:left;transition:transform .3s}
.platform-card:hover{transform:translateY(-6px);box-shadow:var(--shlg);border-color:transparent}
.platform-card:hover::before{transform:scaleX(1)}
.platform-icon{width:72px;height:72px;border-radius:16px;background:var(--pale);display:flex;align-items:center;justify-content:center;font-size:32px;margin:0 auto 20px;transition:.3s}
.platform-card:hover .platform-icon{background:var(--blue);color:#fff}
.platform-card h3{font-size:19px;font-weight:700;color:var(--blue-dark);margin-bottom:12px}
.platform-card p{font-size:14px;color:var(--g600);line-height:1.7;margin-bottom:16px}
.platform-tags{display:flex;flex-wrap:wrap;gap:6px;justify-content:center}
.ptag{background:var(--pale);color:var(--blue);padding:3px 10px;border-radius:12px;font-size:11px;font-weight:600}

/* ── FEATURES STRIP ── */
.features-strip{background:var(--gray-100);}
.features-row{display:flex;gap:24px;justify-content:center;}
.feat-card{background:var(--white);border-radius:var(--radius);padding:28px 24px;text-align:center;box-shadow:var(--shadow-sm);transition:all .3s;flex:1;}
...
.serve-card .serve-icon img{max-width:50px;}

/* ── PROCESS ── */
.process-bg{background:linear-gradient(135deg,var(--blue-dark) 0%,var(--blue-primary) 100%);}
.process-grid{display:flex;position:relative;}
...
.ind-card{background:var(--white);border-radius:var(--r);padding:24px 20px;text-align:center;border:1.5px solid var(--border);transition:.3s;cursor:default}
.ind-card:hover{background:var(--blue);border-color:var(--blue);transform:translateY(-4px)}
.ind-card:hover *{color:#fff!important}
.ind-card .iicon{font-size:32px;margin-bottom:10px;display:block}
.ind-card h4{font-size:14px;font-weight:700;color:var(--blue-dark);margin-bottom:4px}
.ind-card p{font-size:11px;color:var(--g600);line-height:1.5}

/* WHY US */
.why-grid{display:grid;grid-template-columns:1fr 1fr;gap:60px;align-items:center}
.why-main-img{width:100%;border-radius:var(--rl);background:linear-gradient(135deg,var(--pale),var(--border));height:420px;display:flex;align-items:center;justify-content:center;position:relative;overflow:hidden}
.why-float{position:absolute;background:var(--white);border-radius:12px;padding:14px 20px;box-shadow:var(--shmd);display:flex;align-items:center;gap:12px;min-width:180px}
.why-float.tr{top:30px;right:-20px}
.why-float.bl{bottom:30px;left:-20px}
.badge-icon{font-size:28px}
.badge-text strong{display:block;font-size:18px;font-weight:800;color:var(--blue)}
.badge-text span{font-size:11px;color:var(--g600)}
.why-list{list-style:none;display:flex;flex-direction:column;gap:20px;margin:28px 0}
.why-list li{display:flex;gap:16px;align-items:flex-start}
.why-check{width:28px;height:28px;min-width:28px;background:var(--pale);border-radius:8px;display:flex;align-items:center;justify-content:center;color:var(--blue);font-size:13px;margin-top:2px}
.why-list h4{font-size:15px;font-weight:700;color:var(--blue-dark);margin-bottom:4px}
.why-list p{font-size:13px;color:var(--g600);line-height:1.6}
...
.author-avatar{flex:0 0 44px;width:44px;height:44px;border-radius:50%;background:linear-gradient(135deg,var(--blue-primary),var(--blue-accent));display:flex;align-items:center;justify-content:center;color:#fff;font-size:16px;font-weight:700;}
...
/* ── CTA BAND ── */
.cta-band{border-radius:25px;background:linear-gradient(135deg,var(--blue-dark),var(--blue-primary),var(--blue-accent));padding:80px 0;text-align:center;position:relative;overflow:hidden;max-width:1200px!important;margin:0 auto 80px;}
...
.follow-up li{max-width:20%;flex:0 0 auto;}
...
.contact-info:not(:last-child)::after{display:none;}

/* ── RESPONSIVE ── */
@media (max-width:1480px){
    .hero h1{font-size:38px;line-height:1.50;margin-bottom:15px;}
    h2.section-title,.cta-band h2,.shopify-title{font-size:32px;line-height:44px;}
    .conntect--us .consulting--container h3{font-size:32px;line-height:36px;}
    .conntect--us .consulting--container h3 strong{font-size:32px;line-height:36px;}
    .platform-card{padding:26px 18px;}
    .service-card{padding:25px 20px}
    .feat-card .feat-num{font-size:32px;}
    .btn-shopify,.btn-secondary{padding-left:14px;padding-right:14px;}
    .hero-sub{font-size:16px;margin-bottom:20px;max-width:660px;}
}
@media (max-width:1199px){
    .tech-card{padding:14px 9px;}
    .cta-band{max-width:1040px!important;margin-bottom:50px;}
    .testi-card{padding:20px}
    .follow-up li a img{max-width:100%;}
    .feat-card .feat-num{font-size:25px;}
    .features-row{flex-wrap:wrap;}
    .feat-card{flex:calc(50% - 15px);padding:18px 14px;}
    section.conntect--us .contact-info .border-space img{margin-right:10px;max-width:50px;}
    .connect-with-us,section.conntect--us .contact-info .border-space{padding:10px;}
    .hero-trust{flex-wrap:wrap;}
    .hero-trust-item{display:inline-block;}
    .trust-bar .container{gap:34px 40px;justify-content:center;}
    .cta-band,section{padding:45px 0;}
    .partner-sec{font-size:16px;max-width:100%;margin:0 auto 15px;line-height:1.7;}
    .service-card{padding:22px 18px;}
    .hero h1{font-size:36px;line-height:1.30;}
    section.conntect--us.mn_fooer .consulting--container ul li a {
    text-align: center;
}
section.conntect--us.mn_fooer .consulting--container ul li a img {
    margin: auto;
}
    .custom-header nav.navbar.navbar-expand-lg{padding:10px 0;background:#fff;}
}
...
@media (max-width:1024px){
    ...
    .hero-sub{font-size:16px;line-height:1.7;margin-bottom:15px;max-width:100%;}
    .conntect--us .consulting--container h3 strong,.conntect--us .consulting--container h3{font-size:23px;line-height:36px;}
    .hero .container{gap:30px;}
    .hero .container{grid-template-columns:1fr;}
    .hero-form-card{max-width:500px;margin:0 auto;}
    .cta-band{max-width:800px!important;margin-bottom:50px;}
    .shopify-grid{grid-template-columns:1fr;}
    .shopify-float.tr{top:-30px;right:0px;border-left:4px solid #95BF47;}
    .shopify-main-card{padding:20px;}
    .shopify-main-card{padding:20px 20px 50px;}
    .shopify-section{padding-bottom:90px;}
    .shopify-float.bl{bottom:-30px;left:-4px}
    ...
    .hero-actions,.hero-trust,.hero-chips{justify-content:center;}
    .hero{padding:60px 0 50px;text-align:center;margin-top:90px;}
    .hero-trust,.hero-trust-item,
.hero-actions,
    .hero-chips {
    justify-content: center;
}
    .hero{padding:60px 0 50px;margin-top:90px;text-align: center;}
}
@media (max-width:991px){
    ...
    .custom-header nav.navbar.navbar-expand-lg .container {
    padding: 0 20px;
}
}
@media (max-width:767px){
    ...
    .section-pad .btn-primary-custom{margin:10px 0;}
    .service-tags{justify-content:center;}
    .trust-item{flex-direction:column;align-items:center;text-align:center;max-width:calc(50% - 5px);}
    .section-title.text-md-left::after{left:0;right:0;}
    .hero h1{font-size:26px;}
    .cta-band h2,h2.section-title{font-size:24px;line-height:36px;}
    .services-grid,.features-row,.serve-grid,.process-grid,.testi-grid,.compliance-grid{grid-template-columns:1fr;}
...
<section class="hero" id="home">
  ...
  <div class="container">
    <div class="hero-content">
      <div class="hero-badge"><i class="fas fa-star"></i> Australia's Trusted Website Development Company</div>
      <h1>Australia's Best <span>Website Development</span> Company</h1>
      <p class="hero-sub">
        1200+ websites built for Australian businesses. Full-service website design and development company specialising in Shopify, WordPress, custom web development and enterprise solutions across Sydney, Melbourne, Brisbane and Perth.
      </p>
      <div class="hero-chips">
        <div class="chip"><i class="fas fa-check-circle"></i> Shopify Partner</div>
        <div class="chip"><i class="fas fa-check-circle"></i> WordPress Expert</div>
        <div class="chip"><i class="fas fa-check-circle"></i> Custom Web Apps</div>
        <div class="chip"><i class="fas fa-check-circle"></i> eCommerce Specialists</div>
        <div class="chip"><i class="fas fa-check-circle"></i> React / Next.js</div>
      </div>
      <div class="hero-actions">
        <a href="#contact-form" class="btn-white"><i class="fas fa-rocket"></i> Get Free Proposal</a>
        <a href="#shopify" class="btn-secondary" style="border-color:rgba(255,255,255,.5);color:#fff"><i class="fab fa-shopify"></i> Shopify Solutions</a>
      </div>
      <div class="hero-trust">
        <div class="hero-trust-item"><i class="fas fa-star"></i><i class="fas fa-star"></i><i class="fas fa-star"></i><i class="fas fa-star"></i><i class="fas fa-star"></i> <span>4.9/5 on Google (200+ Reviews)</span></div>
        <div class="hero-trust-item"><i class="fas fa-shield-alt"></i> <span>ISO 27001</span></div>
        <div class="hero-trust-item"><i class="fab fa-shopify" style="color:#95BF47"></i> <span>Shopify Partner</span></div>
      </div>
    </div>
    ...
    <div class="hero-form-card" id="contact-form">
      ...
      <div class="form-sub">Australia's top-rated website development company. <span>Response within 2 hours.</span></div>
      <div id="main-form">
                <?php echo do_shortcode('[elementor-template id="34781"]'); ?>
      </div>
      <div class="success-msg" id="success-msg"><i class="fas fa-check-circle"></i><h3>Request Received!</h3><p>Thank you! Our web development team will contact you within 2 business hours.</p></div>
    </div>
  </div>
</section>

<!-- TRUST BAR -->
<section class="trust-sec pb-0">  
<div class="container">
<div class="trust-bar">
  <div class="container">
    <div class="trust-item"><div class="trust-icon"><i class="fas fa-globe"></i></div><div><span class="trust-stat" data-count="1200">0</span>+ Websites Built</div></div>
    <div class="trust-item"><div class="trust-icon"><i class="fas fa-award"></i></div><div><span class="trust-stat" data-count="12">0</span>+ Years Experience</div></div>
    <div class="trust-item"><div class="trust-icon"><i class="fas fa-users"></i></div><div><span class="trust-stat" data-count="250">0</span>+ Developers</div></div>
    <div class="trust-item"><div class="trust-icon"><i class="fas fa-star"></i></div><div><span class="trust-stat">4.9</span>/5 Rating</div></div>
    <div class="trust-item"><div class="trust-icon"><i class="fas fa-map-marker-alt"></i></div><div><span class="trust-stat" data-count="8">0</span> AU Cities</div></div>
  </div>
</div>
</div>
</section>
...
<section id="services">
  <div class="container">
    <div class="section-head text-center reveal">
      <div class="section-label"><i class="fas fa-code"></i> Our Services</div>
      <h2 class="section-title">Website Development <span>Services</span></h2>
      <p class="section-desc">Full-service website design and development company offering custom, WordPress, Shopify and enterprise web solutions for Australian businesses.</p>
    </div>
    <div class="services-grid">
      <div class="service-card reveal"><div class="svc-icon"><i class="fab fa-shopify"></i></div><h3>Shopify Website Development</h3><p>Certified Shopify Partner. Custom theme development, app integrations, Shopify Plus enterprise builds and store migration for Australian eCommerce brands.</p><div class="service-tags"><span class="stag">Shopify</span><span class="stag">Shopify Plus</span><span class="stag">Apps</span></div></div>
      <div class="service-card reveal"><div class="svc-icon"><i class="fab fa-wordpress"></i></div><h3>WordPress Development</h3><p>Custom WordPress website development company services — themes, plugins, WooCommerce, multisite, headless WordPress and enterprise CMS solutions.</p><div class="service-tags"><span class="stag">WordPress</span><span class="stag">WooCommerce</span><span class="stag">Plugins</span></div></div>
      <div class="service-card reveal"><div class="svc-icon"><i class="fas fa-code"></i></div><h3>Custom Web Development</h3><p>Bespoke custom website development using React, Next.js, Vue.js, Laravel and Node.js. Tailored solutions for unique Australian business requirements.</p><div class="service-tags"><span class="stag">React</span><span class="stag">Next.js</span><span class="stag">Laravel</span></div></div>
      <div class="service-card reveal"><div class="svc-icon"><i class="fas fa-building"></i></div><h3>Corporate Websites</h3><p>Professional corporate website development for Australian enterprises — responsive design, CMS integration, brand-aligned experiences and multi-language support.</p><div class="service-tags"><span class="stag">Corporate</span><span class="stag">Responsive</span><span class="stag">CMS</span></div></div>
      <div class="service-card reveal"><div class="svc-icon"><i class="fas fa-shopping-cart"></i></div><h3>eCommerce Development</h3><p>End-to-end eCommerce website development with Shopify, WooCommerce, Magento, payment gateways (Afterpay, Zip), shipping integration and conversion optimisation.</p><div class="service-tags"><span class="stag">eCommerce</span><span class="stag">Afterpay</span><span class="stag">Shipping</span></div></div>
      <div class="service-card reveal"><div class="svc-icon"><i class="fas fa-cloud"></i></div><h3>Web Applications & SaaS</h3><p>Custom web application and SaaS platform development — dashboards, portals, booking systems, APIs, and scalable cloud-native applications on AWS/GCP.</p><div class="service-tags"><span class="stag">SaaS</span><span class="stag">APIs</span><span class="stag">Cloud</span></div></div>
    </div>
  </div>
</section>

<!-- ═══════════════════════════════════════════════════════ -->
<!-- ★★★ SHOPIFY DEDICATED SECTION (MANDATORY) ★★★ -->
<!-- ═══════════════════════════════════════════════════════ -->
<section class="shopify-section" id="shopify">
  <div class="container">
    <div class="shopify-grid">
      <div class="reveal-left">
        <div class="shopify-badge"><i class="fab fa-shopify"></i> Official Shopify Partner</div>
        <h2 class="shopify-title">Australia's Leading <span>Shopify Website</span> Development Company</h2>
        <p class="shopify-desc">
          As an official Shopify Partner and certified Shopify website development company, we've launched 300+ Shopify stores for Australian brands. From Shopify Basic to Shopify Plus enterprise, we build custom themes, develop private apps, integrate Australian payment systems (Afterpay, Zip, Stripe), and optimise stores for Google Ads and conversions.
        </p>
        <div class="shopify-features">
          <div class="sf-item"><div class="sf-check"><i class="fas fa-check"></i></div><div><h4>Custom Shopify Theme Development</h4><p>Pixel-perfect, conversion-optimised themes built from scratch for your brand</p></div></div>
          <div class="sf-item"><div class="sf-check"><i class="fas fa-check"></i></div><div><h4>Shopify Plus Enterprise</h4><p>Enterprise-grade Shopify Plus for high-volume Australian retailers</p></div></div>
          <div class="sf-item"><div class="sf-check"><i class="fas fa-check"></i></div><div><h4>Shopify App Development</h4><p>Custom private and public apps to extend your store's functionality</p></div></div>
          <div class="sf-item"><div class="sf-check"><i class="fas fa-check"></i></div><div><h4>Shopify Hydrogen (Headless)</h4><p>Blazing-fast headless Shopify storefronts with React/Hydrogen</p></div></div>
          <div class="sf-item"><div class="sf-check"><i class="fas fa-check"></i></div><div><h4>Platform Migration to Shopify</h4><p>Seamless migration from WooCommerce, Magento, BigCommerce and more</p></div></div>
          <div class="sf-item"><div class="sf-check"><i class="fas fa-check"></i></div><div><h4>AU Payment Integration</h4><p>Afterpay, Zip Pay, Stripe, Apple Pay and all Australian payment methods</p></div></div>
        </div>
        <div class="shopify-actions">
          <a href="#contact-form" class="btn-shopify"><i class="fab fa-shopify"></i> Get Shopify Quote</a>
          <a href="tel:+61468280070" class="btn-secondary"><i class="fas fa-phone"></i> Talk to Shopify Expert</a>
        </div>
        <div class="shopify-trust">
          <span><i class="fas fa-check-circle"></i> 300+ Shopify Stores</span>
          <span><i class="fas fa-check-circle"></i> Certified Partners</span>
          <span><i class="fas fa-check-circle"></i> Shopify Plus Experts</span>
        </div>
      </div>
      <div class="shopify-visual reveal-right">
        <div class="shopify-main-card">
          <span class="shopify-logo-big"><i class="fab fa-shopify" style="color:#95BF47"></i></span>
          <div class="shopify-card-title">Certified Shopify Partner</div>
          <div class="shopify-card-sub">Custom Shopify website development for Australian brands. From startup stores to Shopify Plus enterprise — we build, migrate and grow your Shopify business.</div>
          <div class="shopify-stats">
            <div class="sstat"><strong data-count="300">0</strong><span>Shopify Stores</span></div>
            <div class="sstat"><strong>4.9★</strong><span>Avg Rating</span></div>
            <div class="sstat"><strong>3.2x</strong><span>Sales Growth</span></div>
          </div>
          <div class="shopify-platforms">
            <div class="sp-badge"><i class="fas fa-store"></i> Shopify Basic</div>
            <div class="sp-badge"><i class="fas fa-building"></i> Shopify Plus</div>
            <div class="sp-badge"><i class="fas fa-bolt"></i> Hydrogen</div>
          </div>
        </div>
        <div class="shopify-float tr"><span class="sf-badge-icon">🏆</span><div class="sf-badge-text"><strong>300+</strong><span>Shopify Stores Built</span></div></div>
        <div class="shopify-float bl"><span class="sf-badge-icon">🇦🇺</span><div class="sf-badge-text"><strong>100%</strong><span>AU Payment Ready</span></div></div>
      </div>
    </div>
  </div>
</section>
...
<section class="tech-bg">
  <div class="container">
    <div class="section-head text-center reveal">
      <div class="section-label"><i class="fas fa-layer-group"></i> Tech Stack</div>
      <h2 class="section-title">Technologies <span>We Work With</span></h2>
    </div>
    <div class="tech-grid">
      <div class="tech-card reveal"><span class="ticon"><i class="fab fa-shopify" style="color:#95BF47"></i></span><h4>Shopify</h4><p>eCommerce Platform</p></div>
      <div class="tech-card reveal"><span class="ticon"><i class="fab fa-wordpress" style="color:#21759b"></i></span><h4>WordPress</h4><p>CMS Platform</p></div>
      <div class="tech-card reveal"><span class="ticon"><i class="fab fa-react" style="color:#61DAFB"></i></span><h4>React & Next.js</h4><p>Frontend Framework</p></div>
      <div class="tech-card reveal"><span class="ticon"><i class="fab fa-node-js" style="color:#339933"></i></span><h4>Node.js</h4><p>Backend Runtime</p></div>
      <div class="tech-card reveal"><span class="ticon"><i class="fab fa-laravel" style="color:#FF2D20"></i></span><h4>Laravel</h4><p>PHP Framework</p></div>
      <div class="tech-card reveal"><span class="ticon"><i class="fab fa-aws" style="color:#FF9900"></i></span><h4>AWS / GCP</h4><p>Cloud Infrastructure</p></div>
      <div class="tech-card reveal"><span class="ticon"><i class="fab fa-python" style="color:#3776AB"></i></span><h4>Python / Django</h4><p>Backend Framework</p></div>
      <div class="tech-card reveal"><span class="ticon"><i class="fab fa-vuejs" style="color:#4FC08D"></i></span><h4>Vue.js / Nuxt</h4><p>Frontend Framework</p></div>
      <div class="tech-card reveal"><span class="ticon"><i class="fab fa-magento" style="color:#EE672F"></i></span><h4>Magento</h4><p>Enterprise Commerce</p></div>
      <div class="tech-card reveal"><span class="ticon"><i class="fas fa-database" style="color:var(--blue)"></i></span><h4>PostgreSQL / MySQL</h4><p>Databases</p></div>
      <div class="tech-card reveal"><span class="ticon"><i class="fab fa-figma" style="color:#F24E1E"></i></span><h4>Figma</h4><p>UI/UX Design</p></div>
      <div class="tech-card reveal"><span class="ticon"><i class="fab fa-docker" style="color:#2496ED"></i></span><h4>Docker / K8s</h4><p>DevOps</p></div>
    </div>
  </div>
</section>
...
<section class="process-bg">
  <div class="container">
    <div class="section-head text-center reveal">
      ...
      <h2 class="section-title" style="color:#fff">How We Build <span style="color:#67e8f9">Your Website</span></h2>
    </div>
    <div class="process-grid">
      <div class="process-step reveal"><div class="step-num">1</div><h3>Discovery</h3><p>Business goals, competitor analysis, technical scoping and platform recommendations for your Australian project.</p></div>
      <div class="process-step reveal"><div class="step-num">2</div><h3>Design</h3><p>Wireframes, UI/UX design, responsive mockups and interactive prototypes reviewed and approved by you.</p></div>
      <div class="process-step reveal"><div class="step-num">3</div><h3>Development</h3><p>Agile sprints, weekly demos, clean code architecture and integration with your business systems.</p></div>
      <div class="process-step reveal"><div class="step-num">4</div><h3>Launch & Grow</h3><p>QA testing, performance tuning, SEO, Google Analytics setup, and ongoing Australian-based support.</p></div>
    </div>
  </div>
</section>
      ...
      <div class="reveal-left">
        <div class="why-main-img">
          <div style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;flex-direction:column;gap:16px">
            ...
            <div style="font-size:18px;font-weight:700;color:var(--blue-dark)">Australia's Trusted Web Dev Company</div>
            <div style="font-size:14px;color:var(--g600);text-align:center;padding:0 40px">1200+ websites built for Australian businesses</div>
          </div>
          <div class="why-float tr"><span class="badge-icon">⭐</span><div class="badge-text"><strong>4.9/5</strong><span>Google Rating</span></div></div>
          <div class="why-float bl"><span class="badge-icon">🇦🇺</span><div class="badge-text"><strong>1200+</strong><span>AU Websites Built</span></div></div>
        </div>
      </div>
      <div class="reveal-right">
        <div class="section-label"><i class="fas fa-trophy"></i> Why Emizentech</div>
        <h2 class="section-title">Why Australian Businesses <span>Choose Us</span></h2>
        <p style="font-size:15px;color:var(--g600);line-height:1.75;margin-bottom:10px">
          As a leading website development company in Australia, we combine technical excellence with deep local market understanding to deliver websites that drive results.
        </p>
        <ul class="why-list">
          <li><div class="why-check"><i class="fas fa-check"></i></div><div><h4>12+ Years & 1200+ Websites</h4><p>Over a decade of experience delivering website development projects across Sydney, Melbourne, Brisbane, Perth and all of Australia — from startups to enterprises.</p></div></li>
          <li><div class="why-check"><i class="fas fa-check"></i></div><div><h4>Full-Stack Technical Expertise</h4><p>250+ developers skilled in Shopify, WordPress, React, Next.js, Laravel, Node.js and enterprise frameworks. We recommend the best technology for your specific needs.</p></div></li>
          <li><div class="why-check"><i class="fas fa-check"></i></div><div><h4>Google Ads & SEO Built-In</h4><p>Every website is optimised for Google Ads Quality Score, Core Web Vitals, and SEO from day one — so you get the best ROI from your digital marketing spend.</p></div></li>
          <li><div class="why-check"><i class="fas fa-check"></i></div><div><h4>Australian-Based Support</h4><p>AEST-hours support team, dedicated project managers, weekly reporting and transparent pricing with no hidden costs. Your satisfaction is guaranteed.</p></div></li>
        </ul>
        <a href="#contact-form" class="btn-primary"><i class="fas fa-calendar"></i> Book Free Strategy Call</a>
      </div>
    </div>
  </div>
</section>
...
<section class="tech-bg">
  <div class="container">
    <div class="section-head text-center reveal">
      <div class="section-label"><i class="fas fa-industry"></i> Industries</div>
      <h2 class="section-title">Industries <span>We Serve</span></h2>
      <p class="section-desc">Website development company with deep expertise across Australia's key industries.</p>
    </div>
    <div class="ind-grid">
      <div class="ind-card reveal"><span class="iicon">🛒</span><h4>Retail & eCommerce</h4><p>Shopify, WooCommerce and custom online stores</p></div>
      <div class="ind-card reveal"><span class="iicon">🏥</span><h4>Healthcare</h4><p>NDIS, telehealth and medical practice websites</p></div>
      <div class="ind-card reveal"><span class="iicon">🏦</span><h4>Finance & Fintech</h4><p>Secure, compliant financial service platforms</p></div>
      <div class="ind-card reveal"><span class="iicon">🎓</span><h4>Education</h4><p>LMS, university and e-learning platforms</p></div>
      <div class="ind-card reveal"><span class="iicon">🏗️</span><h4>Construction & Trade</h4><p>Portfolio and lead generation websites</p></div>
      <div class="ind-card reveal"><span class="iicon">🍔</span><h4>Food & Hospitality</h4><p>Restaurant, hotel and booking websites</p></div>
      <div class="ind-card reveal"><span class="iicon">🏘️</span><h4>Real Estate</h4><p>Property listing and CRM-integrated platforms</p></div>
      <div class="ind-card reveal"><span class="iicon">⚖️</span><h4>Legal & Professional</h4><p>Law firm and professional services websites</p></div>
    </div>
  </div>
</section>

<!-- TESTIMONIALS -->
<section class="testi-bg pb-0" id="testimonials">
  <div class="container">
    <div class="section-head text-center reveal">
      <div class="section-label"><i class="fas fa-quote-left"></i> Client Success</div>
      <h2 class="section-title">What Our <span>Australian Clients</span> Say</h2>
    </div>
    <div class="testi-grid">
      <div class="testi-card reveal">
        <span class="testi-quote">"</span>
        <div class="stars">★★★★★</div>
        <p class="testi-text">Emizentech is the best website development company we've worked with in Australia. They rebuilt our corporate site in 4 weeks and our Google Ads quality scores jumped from 5 to 9. Incredible team.</p>
        <div class="testi-author"><div class="author-avatar">AB</div><div><div class="author-name">Andrew Blake</div><div class="author-role">CEO — NovaTech Solutions, Sydney</div></div></div>
      </div>
      <div class="testi-card reveal">
        <span class="testi-quote">"</span>
        <div class="stars">★★★★★</div>
        <p class="testi-text">We needed a Shopify Plus migration and custom app development. Emizentech handled everything flawlessly — our Shopify store now processes 10x the volume it used to with faster checkout. Highly recommend their Shopify development team.</p>
        <div class="testi-author"><div class="author-avatar">JW</div><div><div class="author-name">Jessica Wong</div><div class="author-role">eCommerce Director — GreenLeaf AU, Melbourne</div></div></div>
      </div>
      <div class="testi-card reveal">
        <span class="testi-quote">"</span>
        <div class="stars">★★★★★</div>
        <p class="testi-text">Our custom web application connects our field teams across 6 Australian states. Emizentech built it with React and Node.js — it's fast, reliable, and the ongoing support from their Australian team has been outstanding.</p>
        <div class="testi-author"><div class="author-avatar">PK</div><div><div class="author-name">Peter Kovacs</div><div class="author-role">CTO — FieldForce Australia, Brisbane</div></div></div>
      </div>
    </div>
  </div>
</section>
...
<section id="faq">
  <div class="container">
    ...
    <div class="faq-list">
      <div class="faq-item"><div class="faq-q" onclick="toggleFaq(this)"><h4>How much does website development cost in Australia?</h4><div class="faq-icon"><i class="fas fa-plus"></i></div></div><div class="faq-a">Website development in Australia ranges from $2,000 for a basic WordPress site to $100,000+ for enterprise custom web applications. A typical business website costs $5,000–$20,000, while Shopify eCommerce stores range from $5,000–$30,000. We provide free detailed quotes after understanding your requirements.</div></div>
      <div class="faq-item"><div class="faq-q" onclick="toggleFaq(this)"><h4>Why should I choose Emizentech as my website development company?</h4><div class="faq-icon"><i class="fas fa-plus"></i></div></div><div class="faq-a">We're a top-rated website development company in Australia with 12+ years, 1200+ websites, 250+ developers, and a 4.9/5 Google rating. We're a certified Shopify Partner and deliver across all major technologies. Our Australian-based project managers ensure transparent communication and on-time delivery.</div></div>
      <div class="faq-item"><div class="faq-q" onclick="toggleFaq(this)"><h4>Do you offer Shopify website development?</h4><div class="faq-icon"><i class="fas fa-plus"></i></div></div><div class="faq-a">Yes — we're an official Shopify Partner and one of Australia's leading Shopify website development companies. We've built 300+ Shopify stores including Shopify Plus enterprise builds, custom theme development, app development, and seamless migrations from WooCommerce, Magento and BigCommerce to Shopify.</div></div>
      <div class="faq-item"><div class="faq-q" onclick="toggleFaq(this)"><h4>How long does it take to build a website?</h4><div class="faq-icon"><i class="fas fa-plus"></i></div></div><div class="faq-a">A standard business website takes 3–6 weeks. Shopify eCommerce stores take 4–8 weeks. Custom web applications and enterprise platforms take 8–20 weeks depending on complexity. We provide milestone-based timelines before work begins and keep you updated weekly.</div></div>
      <div class="faq-item"><div class="faq-q" onclick="toggleFaq(this)"><h4>Do you develop websites for businesses outside Sydney and Melbourne?</h4><div class="faq-icon"><i class="fas fa-plus"></i></div></div><div class="faq-a">Absolutely. While we have a strong presence in Sydney and Melbourne, we serve clients across all of Australia including Brisbane, Perth, Adelaide, Gold Coast, Canberra, Hobart and regional areas. Our project management process is fully remote-friendly with video calls, shared project dashboards and AEST-hours support.</div></div>
      <div class="faq-item"><div class="faq-q" onclick="toggleFaq(this)"><h4>Do you provide ongoing website maintenance and support?</h4><div class="faq-icon"><i class="fas fa-plus"></i></div></div><div class="faq-a">Yes. We offer flexible maintenance packages covering security updates, performance monitoring, backups, content updates, bug fixes and new feature development. Our support team is available during AEST hours with 24/7 emergency support for critical issues. Plans start from $500/month.</div></div>
    </div>
  </div>
</section>
...
<section class="cta-band">
  <div class="container">
    ...
    <p>Join 1200+ Australian businesses that trust Emizentech for website development. Get your free consultation and proposal today.</p>
    <div class="cta-actions">
      <a href="#contact-form" class="btn-white"><i class="fas fa-rocket"></i> Get Free Proposal</a>
      <a href="#shopify" class="btn-shopify"><i class="fab fa-shopify"></i> Shopify Solutions</a>
 ...
 <section class="conntect--us mn_fooer">
    ...
    <div class="watsappic">
        <a href="https://wa.me/61468280070" target="_blank" id="whatsapp-link" rel="nofollow">
...
function animateCounters(){document.querySelectorAll('[data-count]').forEach(el=>{const t=+el.dataset.count;const s=t/60;let c=0;const timer=setInterval(()=>{c+=s;if(c>=t){c=t;clearInterval(timer)}el.textContent=Math.floor(c)},20)})}
const cObs=new IntersectionObserver(e=>{if(e[0].isIntersecting){animateCounters();cObs.disconnect()}});
cObs.observe(document.querySelector('.trust-bar'));
...
function handleSubmit(e){e.preventDefault();document.getElementById('main-form').style.display='none';document.getElementById('success-msg').style.display='block'}

/*setTimeout(()=>{const t=document.getElementById('toast');t.classList.add('show');setTimeout(()=>t.classList.remove('show'),4000)},6000);*/

/*window.addEventListener('scroll',()=>{document.querySelector('header').style.boxShadow=window.scrollY>50?'0 4px 30px rgba(18,70,190,.15)':''});*/

document.querySelectorAll('.form-group input,.form-group select,.form-group textarea').forEach(el=>{
  el.addEventListener('focus',()=>{const i=el.parentElement.querySelector('.fi');if(i)i.style.color='var(--blue)'});
  el.addEventListener('blur',()=>{const i=el.parentElement.querySelector('.fi');if(i)i.style.color='var(--blue-light)'});
});
</script>
